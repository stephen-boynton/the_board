...
public class BoardManager : MonoBehaviour
{
  ...
  private void UpdateSelection()
  {
    ...
    if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 40.0f, LayerMask.GetMask("BoardPlane")))
    {
      ...
      MakeTileVisible(currentSelection.transform);
      ...
      {
        MakeTileInvisible(previousSelection.transform);
        Debug.Log("I am invisible? No :(");
        previousSelection = currentSelection;
      }
    }
  }
  ...
  private void SpawnAllTiles()
  {
    for (int i = 0; i < size; i++)
    {
      for (int j = 0; j < size; j++)
      {
        ...
        MakeTileInvisible(goS.transform.GetChild(0));
      }
    }
  }

  private void MakeTileVisible(Transform plane)
  {
    plane.GetComponent<Renderer>().enabled = true;
  }

  private void MakeTileInvisible(Transform plane)
  {
