...
public class GridManager : MonoBehaviour {
    ...
    Color selectedCellColor = new Color (0, 1, 1, 1);
    Color defaultCellColor = new Color (1, 1, 1, 1);
    ...
    public void SelectCells (List<Cell> cells) {
        for (int i = cells.Count - 1; i >= 0; --i)
            cells[i].transform.GetComponent<Renderer> ().material.SetColor ("_Color", selectedCellColor);
    }

    public void DeSelectCells (List<Cell> cells) {
        for (int i = cells.Count - 1; i >= 0; --i)
            cells[i].transform.GetComponent<Renderer> ().material.SetColor ("_Color", defaultCellColor);
    }
}